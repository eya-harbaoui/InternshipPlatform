...
updateOfferDraft: async (req, res) => {
    try {
      ...
      if (offer.status !== "brouillon") {
        return res.status(400).json({ message: "Cannot update offer in this status" });
      }
  ...
  validateOffer: async (req, res) => {
    try {
      ...
      if (offer.status !== "en cours de validation") {
        return res.status(400).json({ message: "Offer is not in validation status" });
      }
      offer.status = "publié";
      await offer.save();
  ...
  getPublishedOffersForStudents: async (req, res) => {
    try {
      const publishedOffers = await Offer.find({ status: "publié" });
  ...
  putOfferInDraft: async (req, res) => {
    try {
      ...
      if (offer.status === "publié") {
        return res.status(400).json({ message: "Cannot put published offer in draft" });
      }
  ...
  deleteOfferDraft: async (req, res) => {
    try {
      ...
      if (offer.status !== "brouillon") {
        return res.status(400).json({ message: "Cannot delete offer in this status" });
      }
      await offer.remove();
...
gethistorique_offres : async (req, res) => {
  try {
      ...
      const historicalOffers = await Offer.find({ status: { $in: ['archivé', 'publié'] } });
